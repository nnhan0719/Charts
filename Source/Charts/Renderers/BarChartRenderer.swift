...
//                // edited by nhan
//                let bezierPath = UIBezierPath(roundedRect: barRect, cornerRadius: CGFloat(10))
...
//                context.setFillColor(dataSet.color(atIndex: j).cgColor)
            ...
            if j == 3 {
                ...
                // edited by nhan - gradient
                context.setAlpha(1.0)
                //#88FF98
                let topColor = dataSet.color(atIndex: j).cgColor == UIColor(red: 0/255, green: 198/255, blue: 91/255, alpha: 1.0).cgColor ? UIColor(red: 136/255, green: 255/255, blue: 152/255, alpha: 1).cgColor : UIColor.white.cgColor
                
                let fillColors = [dataSet.color(atIndex: j).cgColor, topColor]
            ...
            // draw dash line
            // edited by nhan
            if j == buffer.rects.count - 1 && dataProvider.barData != nil {
                let  path = UIBezierPath()
                let barRect = buffer.rects[j]
                let dataSetItem = dataSet.entryForIndex(j)
                let  p0 = CGPoint(x: 50, y: barRect.origin.y)
                path.move(to: p0)
                
                let  p1 = CGPoint(x: 200, y: barRect.origin.y)
                path.addLine(to: p1)
                ...
                UIColor(red: 131/255, green: 131/255, blue: 131/255, alpha: 1.0).set()
                path.stroke()

                if (dataSetItem?.y ?? 0) > 0 {
                    let roundAmount = UIBezierPath(roundedRect: CGRect(x: 15, y: barRect.origin.y - 12 , width: 92, height: 24), byRoundingCorners: [.allCorners], cornerRadii: CGSize(width: 5, height: 5))
                    roundAmount.addClip()
                    UIColor(red: 131/255, green: 131/255, blue: 131/255, alpha: 1.0).setFill()
                    roundAmount.fill()
                    ...
                    let number = NSNumber(floatLiteral: dataSetItem?.y ?? 0)
                    ...
                    let text = "\(((value?.replacingOccurrences(of: ".00", with: "") ?? "0").replacingOccurrences(of: "$", with: "")).replacingOccurrences(of: dataProvider.isDrawBarShadowEnabled ? "," : ".", with: dataProvider.isDrawBarShadowEnabled ? "." : ","))"
                    ChartUtils.drawText(context: context, text: text, point: CGPoint(x: 60, y: barRect.origin.y - 8), align: .center, attributes: attributes)
                }
            }
